...
function Play({ project, refresh, nav, setError }: any) {
  ...
  const trayX = 990, trayY = 128;
  function pileX(idx: number) { return trayX + 118 + ((idx * 47) % 160) - 80; }
  function pileY(idx: number) { return trayY + 210 + ((idx * 83) % 230) - 115; }
  function pileRot(idx: number) { return [0, 90, 180, 270][idx % 4]; }
  const [pieceStates, setPieceStates] = useState<Record<string, PieceState>>(() => Object.fromEntries(pieces.map((p: Piece, idx: number) => [p.id, { x: snap.pieces?.[p.id]?.x ?? pileX(idx), y: snap.pieces?.[p.id]?.y ?? pileY(idx), rotation: snap.pieces?.[p.id]?.rotation ?? pileRot(idx), placed: snap.pieces?.[p.id]?.placed ?? false }] as const))); 
  ...
  function trySnap(id: string) { const pc = pieces.find((p: Piece) => p.id === id); const st = pieceStates[id]; if (!pc || !st) return false; const targetX = boardX + pc.targetX, targetY = boardY + pc.targetY; const dx = st.x - targetX, dy = st.y - targetY; const dist = Math.hypot(dx, dy); const rotOk = rotDistance(st.rotation) <= Math.max(10, tolerance * 0.35); if (dist <= tolerance && rotOk) { updatePiece(id, { x: targetX, y: targetY, rotation: 0, placed: true }); setSaved('Teil eingerastet ✓'); return true; } return false; }
