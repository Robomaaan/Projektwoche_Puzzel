...
function Play({ project, refresh, nav, setError }: any) {
  ...
  function trySnap(id: string) { const pc = pieces.find((p: Piece) => p.id === id); const st = pieceStates[id]; if (!pc || !st) return false; const targetX = boardX + pc.targetX * scale, targetY = boardY + pc.targetY * scale; const dx = st.x - targetX, dy = st.y - targetY; const dist = Math.hypot(dx, dy); const rotOk = rotDistance(st.rotation) <= Math.max(10, tolerance * 0.35); if (dist <= tolerance && rotOk) { const wasComplete = progressFor(pieceStates) >= 100; const nextStates = { ...pieceStates, [id]: { ...st, x: targetX, y: targetY, rotation: 0, placed: true } }; const isComplete = progressFor(nextStates) >= 100; setPieceStates(nextStates); setSaved(isComplete ? 'Puzzle fertig ✓ Speichere Abschluss…' : 'Teil eingerastet ✓ Speichere automatisch…'); void persistProgress(nextStates, true); if (isComplete && !wasComplete) { setCompletedSeconds(seconds); setFinishOpen(true); } return true; } return false; }
