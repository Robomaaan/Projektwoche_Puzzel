...
export function isRotationAligned(rotation: number, toleranceDegrees = 18) {
  const n = ((rotation % 360) + 360) % 360;
  return Math.min(n, 360 - n) <= toleranceDegrees;
}
...
function Play({ project, refresh, nav, setError }: any) {
  ...
  useEffect(() => { const t = setInterval(() => setSeconds((sec: number) => finishOpen ? sec : sec + 1), 1000); return () => clearInterval(t); }, [finishOpen]);
  useEffect(() => {
  ...
  function targetFor(pc: Piece) { return { x: boardX + pc.targetX * scale, y: boardY + pc.targetY * scale }; }
  function isPieceCorrect(pc: Piece, st?: PieceState) {
    if (!st) return false;
    if (st.placed) return true;
    const target = targetFor(pc);
    const dist = Math.hypot(st.x - target.x, st.y - target.y);
    const completionTolerance = Math.max(tolerance, Math.min(displayPieceW, displayPieceH) * 0.42);
    return dist <= completionTolerance && isRotationAligned(st.rotation, Math.max(16, tolerance * 0.4));
  }
  function solvedCountFor(states: Record<string, PieceState>) { return pieces.filter((pc: Piece) => isPieceCorrect(pc, states[pc.id])).length; }
  function isPuzzleSolved(states: Record<string, PieceState>) { return pieces.length > 0 && solvedCountFor(states) === pieces.length; }
  function normalizedSolvedStates(states: Record<string, PieceState>) { return Object.fromEntries(pieces.map((pc: Piece) => { const target = targetFor(pc); const st = states[pc.id]; return [pc.id, isPieceCorrect(pc, st) ? { ...(st ?? { rotation: 0 }), x: target.x, y: target.y, rotation: 0, placed: true } : st] as const; })); }
  const placedCount = solvedCountFor(pieceStates); const progress = pieces.length ? placedCount / pieces.length * 100 : 0;
  function progressFor(states: Record<string, PieceState>) { return pieces.length ? solvedCountFor(states) / pieces.length * 100 : 0; }
  ...
  function openFinish(states: Record<string, PieceState>) { const done = normalizedSolvedStates(states); setPieceStates(done); setSaved('Puzzle fertig ✓ Speichere Abschluss…'); if (completedSeconds === null) setCompletedSeconds(seconds); setFinishOpen(true); void persistProgress(done, true); }
  function checkCompletion(states = pieceStates) { if (!isPuzzleSolved(states)) { setSaved('Noch nicht fertig: Einige Teile liegen noch nicht exakt genug oder sind verdreht.'); return false; } openFinish(states); return true; }
  function trySnap(id: string) { const pc = pieces.find((p: Piece) => p.id === id); const st = pieceStates[id]; if (!pc || !st) return false; const target = targetFor(pc); const dx = st.x - target.x, dy = st.y - target.y; const dist = Math.hypot(dx, dy); const rotOk = rotDistance(st.rotation) <= Math.max(10, tolerance * 0.35); if (dist <= tolerance && rotOk) { const nextStates = { ...pieceStates, [id]: { ...st, x: target.x, y: target.y, rotation: 0, placed: true } }; setPieceStates(nextStates); if (isPuzzleSolved(nextStates)) { openFinish(nextStates); } else { setSaved('Teil eingerastet ✓ Speichere automatisch…'); void persistProgress(nextStates, true); } return true; } return false; }
  ...
  useEffect(() => { if (!finishOpen && completedSeconds === null && isPuzzleSolved(pieceStates)) openFinish(pieceStates); }, [pieceStates]);
  const elapsed = formatElapsed(seconds);
  return <div className="play fullscreen-play"><button className="exit-play" title="Puzzlemodus verlassen (ESC)" onClick={() => nav('dashboard')}>×</button><header className="top playtop"><div><h1>{project.title}</h1><p>{pieces.length} Teile · {project.visibility === 'public' ? 'Für alle Nutzer' : 'Privat'} · Teile rechts auswählen/ziehen, Scrollrad dreht in 90°-Schritten</p></div><b className="timer">{elapsed}</b><button onClick={() => save(false)}>Save</button><span className="save-state">{saved}</span></header><div className="playtools"><span>{placedCount}/{pieces.length} eingerastet</span><label>Akzeptanzbereich <input type="range" min="18" max="80" value={tolerance} onChange={e => setTolerance(Number(e.target.value))} /> {tolerance}px</label><button onClick={shuffleTray}>Teilehaufen neu mischen</button><button onClick={() => setShowPreview(!showPreview)}>{showPreview ? 'Puzzlebild ausblenden' : 'Puzzlebild einblenden'}</button><button className="primary" onClick={() => checkCompletion()}>Fertig prüfen</button></div><div className="workspace" onWheel={e => { e.preventDefault(); rotateSelected(e.deltaY); }}><div className={showPreview ? 'target' : 'target hidden-preview'} style={{ left: boardX, top: boardY, backgroundImage: showPreview ? `url(${previewUrl})` : 'none', transform: `scale(${zoom / 100})` }} /><div className="tray pile-tray"><b>Teilehaufen</b><small>Alle Teile liegen absichtlich übereinander. Ziehe ein Teil aus dem Haufen ins Puzzle.</small></div>{pieces.map((pc: Piece) => { const st = pieceStates[pc.id]; return <PieceSvg key={pc.id} pc={pc} st={st?.placed ? { ...st, x: boardX + pc.targetX * scale, y: boardY + pc.targetY * scale } : st} pieceW={pieceW} pieceH={pieceH} knob={knob} scale={scale} rows={cfg.rows} columns={cfg.columns} previewUrl={previewUrl} selected={selected === pc.id} onPointerDown={(e: React.PointerEvent) => onPointerDown(e, pc.id)} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onDoubleClick={() => { updatePiece(pc.id, { rotation: 0 }); setTimeout(() => trySnap(pc.id), 0); }} />; })}</div><div className="bottom"><button onClick={() => alert('Hint: Wenn ein Teil nah genug am Ziel ist und fast richtig gedreht wurde, richtet es sich automatisch aus und rastet grün ein.')}>Hint</button><div className="strip">{pieces.slice(0, 20).map((pc: Piece) => <button key={pc.id} className={selected === pc.id ? 'mini active' : 'mini'} onClick={() => select(pc.id)}>{pc.row + 1}/{pc.column + 1}</button>)}</div><button onClick={() => setZoom(z => Math.max(45, z - 10))}><ZoomOut /></button><b>{zoom}%</b><button onClick={() => setZoom(z => Math.min(130, z + 10))}><ZoomIn /></button><button onClick={() => setZoom(88)}>Fit</button></div>{finishOpen && <div className="finish-backdrop" role="dialog" aria-modal="true" aria-labelledby="finish-title"><div className="finish-modal"><div className="finish-badge">✓</div><h2 id="finish-title">Puzzle geschafft!</h2><p>Alle Teile sitzen richtig. Deine Spielzeit:</p><strong className="finish-time">{formatElapsed(completedSeconds ?? seconds)}</strong><div className="finish-actions"><button onClick={() => setImagePreviewOpen(true)}>Ansehen</button><a className="button-like" href={previewUrl} download={`${project.title || 'puzzle'}-bild`}>Download</a><button className="primary" onClick={() => { setFinishOpen(false); nav('dashboard'); }}>Exit zum Dashboard</button></div></div></div>}{imagePreviewOpen && <div className="image-preview-backdrop" role="dialog" aria-modal="true"><div className="image-preview-modal"><button className="image-preview-close" onClick={() => setImagePreviewOpen(false)}>×</button><h2>Originalbild ansehen</h2><img src={previewUrl} alt={`Originalbild von ${project.title}`} /><a className="button-like primary" href={previewUrl} download={`${project.title || 'puzzle'}-bild`}>Bild herunterladen</a></div></div>}</div>;
}
