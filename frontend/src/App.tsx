...
function Play({ project, refresh, nav, setError }: any) {
  ...
  function trySnap(id: string) { const pc = pieces.find((p: Piece) => p.id === id); const st = pieceStates[id]; if (!pc || !st) return false; const targetX = boardX + pc.targetX * scale, targetY = boardY + pc.targetY * scale; const dx = st.x - targetX, dy = st.y - targetY; const dist = Math.hypot(dx, dy); const rotOk = rotDistance(st.rotation) <= Math.max(10, tolerance * 0.35); if (dist <= tolerance && rotOk) { updatePiece(id, { x: targetX, y: targetY, rotation: 0, placed: true }); setSaved('Teil eingerastet ✓'); return true; } return false; }
