...
async function ensureBucket(admin) {
  const allowedMimeTypes = ['image/jpeg', 'image/png', 'image/webp', 'image/gif', 'application/json'];
  const { error } = await admin.storage.getBucket(BUCKET);
  if (!error) {
    await admin.storage.updateBucket(BUCKET, { public: false, fileSizeLimit: 10 * 1024 * 1024, allowedMimeTypes });
    return true;
  }
  const created = await admin.storage.createBucket(BUCKET, { public: false, fileSizeLimit: 10 * 1024 * 1024, allowedMimeTypes });
