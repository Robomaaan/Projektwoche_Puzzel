...
export async function generatePuzzle(ownerId: string, input: { imageId:string; title:string; rows:number; columns:number; difficulty:string; pieceShape:string; visibility?: 'private' | 'public' }) {
  ...
  await sharp(image.storagePath).resize({ width: 900, height: 600, fit: 'cover' }).webp({ quality: 88 }).toFile(previewPath);
  ...
  for (let r = 0; r < rows; r++) for (let c = 0; c < columns; c++) {
    ...
    piecesData.push({ row: r, column: c, targetX: c * pieceWidth, targetY: r * pieceHeight, currentX: startX + (idx % 3) * (pieceWidth * 0.55), currentY: startY + Math.floor(idx / 3) * (pieceHeight * 0.45), imagePath: piecePath, isPlaced: false });
  }
  await prisma.generatedPuzzle.create({ data: { projectId: project.id, previewPath, status: 'generated', pieces: { create: piecesData } }, include: { pieces: true } });
